...
Usage: $(basename "$0") <options>
    -h, --help                      Display help
    --source-file                   Original HelmRelease
    --target-file                   New HelmRelease
    --helmrepositories-path         Path to where HelmRepositories are located
...
parse_command_line() {
    while :; do
        case "${1:-}" in
            ...
            --target-file)
                ...
                ;;
            --helmrepositories-path)
                if [[ -n "${2:-}" ]]; then
                    helmrepositories_path="$2"
                    shift
                else
                    echo "ERROR: '--helmrepositories-path' cannot be empty." >&2
                    show_help
                    exit 1
                fi
                ;;
    ...
    if [[ -z "${helmrepositories_path}" ]]; then
        echo "ERROR: '--helmrepositories-path' is required." >&2
        show_help
        exit 1
    fi
...
entry() {
    ...
    source_chart_registry_url=$(chart_registry_url "${source_file}" "${helmrepositories_path}")
    ...
    target_chart_registry_url=$(chart_registry_url "${target_file}" "${helmrepositories_path}")
...
chart_registry_url() {
    ...
    find . -path "${helmrepo_path}" -type f -print0 | while IFS= read -r -d "" file; do
